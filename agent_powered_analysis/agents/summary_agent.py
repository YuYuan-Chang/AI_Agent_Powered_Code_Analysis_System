...
class SummaryAgent:
    ...
    def generate_summary(self, query_result: QueryResult, original_intent: str) -> str:
        """
        Generate one-sentence summaries for each component found in the codebase analysis.
        
        Args:
            query_result: The codebase analysis results
            original_intent: The original query or request
            
        Returns:
            One-sentence summaries for each component with their full names and metadata
        """
        try:
            ...
            self.logger.info(f"Generated component summaries for {len(query_result.records)} analysis results: {len(summary)} characters")
            
            return summary
            
        except Exception as e:
            self.logger.error(f"Summary generation failed: {str(e)}")
            return f"Unable to generate component summaries: {str(e)}"
    ...
    def _create_summary_prompt(self, original_intent: str, structured_data: str, total_results: int) -> str:
        """
        Create the prompt for generating one-sentence summaries of code components.
        ...
        Returns:
            Formatted prompt for generating component summaries
        """
        prompt = f"""You are analyzing code components and need to provide one-sentence summaries for each component found in the codebase.

Query: "{original_intent}"

Codebase Analysis Results:
- Total Analysis Results: {total_results}
- Components Found: {structured_data}

For each component, provide a one-sentence summary in this exact format:
<full_name>: <one sentence describing what the component does>

Where <full_name> should include the complete qualified name (e.g., backend.airweave.core.exceptions.NotFoundException) and the sentence should concisely describe the component's purpose or functionality.

Examples:
- backend.airweave.core.exceptions.NotFoundException: Represents an exception thrown when a requested resource cannot be found in the system
- utils.database.connection.DatabaseManager: Manages database connections and handles connection pooling for the application
- api.controllers.user.UserController: Handles HTTP requests related to user management operations

Provide summaries for all components found in the analysis results."""
